...
def build_app(
    ...
    print(f"Using static_dir: {static_dir}")
    app.server.static_folder = static_dir
    ...
    app.layout = html.Div([
        ...
        html.Div([
            ...
            html.Span("Overlays: ", style={"marginLeft": "24px", "marginRight": "8px"}),
            dcc.Checklist(
    ...
    app.clientside_callback(
        ...
        function(basename) {
            if (!basename) {
                return "Thailand Fire Danger";
            }
            return "Thailand Fire Danger - Latest Data: " + basename;
        }
        """,
    ...
    def update_map(metric, admin_level, overlays_selected):
        ...
        # Overlays (as line layers)
        overlay_colors = {
            "Protected Areas": "rgba(0,128,0,0.7)",
            "Reserved Forests": "rgba(0,0,255,0.7)",
            "Forest Groups": "rgba(255,140,0,0.7)",
        }
        if overlays and overlays_selected:
            for name in overlays_selected:
                gj_layer = overlays.get(name)
                if not gj_layer:
                    continue
                for feat in gj_layer["features"]:
                    geom = feat["geometry"]
                    color = overlay_colors.get(name, "rgba(0,0,0,0.7)")
    ...
    def export_kml(n_clicks, admin_level, metric):
        ...
        for _, row in gdf.iterrows():
            ...
            def add_poly(poly_geom):
                pol = kml.newpolygon(name=placemark_name)
                xs, ys = poly_geom.exterior.xy
                pol.outerboundaryis = list(zip(xs, ys))
            ...
            if geom.geom_type == "Polygon":
                ...
                last_poly.style.polystyle.outline = 1
...
def main():
    ...
    # Ensure GADM file exists; download if missing (UCDavis mirror)
    ...
    try:
        overlays = {
            "Protected Areas": load_geojson_layer(
                os.path.join(overlays_dir, "protected_areas.geojson")
            ),
            "Reserved Forests": load_geojson_layer(
                os.path.join(overlays_dir, "reserved_national_reserved_forest.geojson")
            ),
            "Forest Groups": load_geojson_layer(
                os.path.join(overlays_dir, "forests_14_groups.geojson")
            ),
        }
    except Exception as e:
